...
loons_dive_2024_mean <- loons_dive_2024 %>% 
  group_by(tide) %>% 
  mutate(mean_tide = round(mean(dive_time_obs), digits = 2))
loons_dive_2024_mean$tide <- factor(loons_dive_2024_mean$tide, levels = c("high_ebb", "mid_ebb", "low_ebb", "low_slack", "low_flood","mid_flood", "high_flood", "high_slack"))
n_fun <- function(x){
  return(data.frame(y = (median(x) + 10), label = round(mean(x), digits = 2)))
}
loons_dive_2024_mean %>% 
ggplot(mapping = aes(x = tide, y = dive_time_obs, fill = tide)) + 
  geom_boxplot() +
  ...
  #geom_text(aes(x = names(meds), y = meds, label = mean_tide), position = position_dodge(width = 0.9), vjust = -1.3)
...
loons_dive_low_2024 %>% 
  ...
  labs(#title = "Dive Times vs. Meters Offshore", 
       ...
       y = "Dive Time (seconds)") +
  stat_smooth(method = "lm", fullrange = T, color = "black", fill = "chartreuse4") +
  scale_x_continuous(expand = c(0,0)) +
  theme(panel.background = element_rect(fill = "#e4f1e2",
                                        color = "black"))
...
loons_dive_2024_gis_mid <- loons_dive_2024_gis %>% 
  filter(general_tide == "mid")

water_depth_mid_lm <- lm(dive_time_obs~water_depth_m, data = loons_dive_2024_gis_mid)
summary(water_depth_mid_lm)

# high specific
loons_dive_2024_gis_high <- loons_dive_2024_gis %>% 
  filter(general_tide == "high")

water_depth_high_lm <- lm(dive_time_obs~water_depth_m, data = loons_dive_2024_gis_high)
summary(water_depth_high_lm)
```
...
```{r}
loons_dive_2024 %>% 
  mutate(mean_dive = mean(dive_time_obs))
```
