...
If we want to compare the abundance of diving behavior between the three tidal stages (high tide, mid tide, and low tide), we need to compare the means of a count variable (number of diving behaviors) across different groups (tidal stages). One suitable test for this scenario is the Poisson regression analysis.

```{r}
#loon_2023_behavior <-

loons_2023 %>% 
  select(species, behavior, date, tide) %>% 
  filter(species == "COMMON LOON") %>% 
  filter(behavior == "DIVING") %>% 
  group_by(date, tide, behavior) %>% 
  count(behavior)
```
