...
```{r}
#table(loons_2023$tide, loons_2024$behavior) #Sowy just needed  to comment this out to knit
```
...
First, we need to calculate adjusted residuals. 

```{r}
adjusted_res1 <- residuals(chi_sq_test1, type = "pearson")
```

Then, we can identify significant differences. 

```{r}
# Identify significant differences
significant_diff1 <- abs(adjusted_res1) > 2
significant_diff1
```
...
```{r}
# Visualize results
heatmap(adjusted_res1, main = "Adjusted Residuals Heatmap")
```

```{r}
# Create a plot of adjusted residuals
barplot(adjusted_res1, main = "Adjusted Residuals", beside = TRUE)
```

#### Second Method

```{r}
summary(table(loons_2023$tide, loons_2023$behavior))
```
...
assocstats(table(loons_2023$tide, loons_2023$behavior))

install.packages("ggplotstats")
library(ggstatsplot)
ggbarstats(
  data = loons_2023,
  x = behavior,
